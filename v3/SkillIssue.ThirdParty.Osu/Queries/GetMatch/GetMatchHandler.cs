...
using SkillIssue.ThirdParty.Osu.Queries.GetMatch.Contracts.Match.Events;

namespace SkillIssue.ThirdParty.Osu.Queries.GetMatch;
...
public class GetMatchHandler(HttpClient client, OsuRequestBuilder requestBuilder)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return async _ =>
        {
            ...
            var nextCursorEventId = FindNextEventIdCursor(matchFrame.Frame, currentEventId);
            ValidateCursorIncreased(currentEventId, nextCursorEventId);

            matchFrame.LastEventTimestamp = FindLastTimestampUpdate(matchFrame.Frame);
    ...
    private bool HasNextMatchFrameInScope(MatchFrameRaw frame)
    {
        ...
        if (frame?.Frame is null) return true;
        if (frame.Frame.Events.Count == 0) return false;

        var firstFrameEvent = frame.Frame.Events[0].EventId;
        var lastFrameEvent = frame.Frame.Events.Last().EventId;

        var nextFrameEvent = FindNextEventIdCursor(frame.Frame, firstFrameEvent);
